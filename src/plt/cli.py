...
def project(
    action: str = typer.Option(
        ..., "--action", help="Action to perform (create, delete)"
    ),
    ...
    try:
        if action == "list":
            ...
            result = bitbucket.project.grant_user_permission(
                workspace, key, user_uuid, permission
            )
            typer.echo(result)
        elif action == "revoke-user-permissions":
            result = bitbucket.project.revoke_user_permissions(
...
def repository(
    ...
    description: Annotated[
        str | None, typer.Option(help="Repository description")
    ] = None,
    ...
    try:
        if action == "create":
            result = bitbucket.repository.create(
                workspace, key, project_key, is_private, description
            )
            typer.echo(result)
        elif action == "delete":
            ...
            result = bitbucket.repository.grant_user_permission(
                workspace, key, user_uuid, permission
            )
            typer.echo(result)
        elif action == "revoke-user-permissions":
...
def workspace(
    action: str = typer.Option(
        ..., "--action", help="Action to perform (list-permissions)"
    ),
    workspace: str = typer.Option(..., "--workspace", help="Bitbucket workspace"),
):
    bitbucket = Bitbucket()
